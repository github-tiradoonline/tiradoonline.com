...
namespace tiradoonline.ClassLibrary
{
    public class clsSQLServer
    {
        ...
        public static string SQLBlankSpaceEncode(string objectname)
        {
        ...
        public void ExportTablesToExcel()
        {
            ...
            foreach (DataTable table in ds.Tables)
            {
                ...
                if (table.Rows.Count > 0)
                {
                    ...
                    //Excel.Range headerRange = excelWorkSheet.get_Range("A5:G5");
                    //headerRange.Interior.Color = XlRgbColor.
        ...
        public void ExportStoredProceduresToExcel()
        {
            ...
            for (int x = 0; x < ors.Rows.Count; x++)
            {
                ...
                if (StoredProcedureName != tempStoredProcedureName)
                {
                    ...
                    //excelWorksheet.Name = StoredProcedureName;
                    excelWorksheet.Name = "Table" + x.ToString();

                    excelWorkSheet = (Excel.Worksheet)excelWorkBook.Worksheets.get_Item(tableCounter + 1);
                    Excel.Range aRange = excelWorkSheet.get_Range("A1:G3");
                    aRange.Borders.get_Item(Excel.XlBordersIndex.xlEdgeLeft).LineStyle = Excel.XlLineStyle.xlContinuous;
                    aRange.Borders.get_Item(Excel.XlBordersIndex.xlEdgeRight).LineStyle = Excel.XlLineStyle.xlContinuous;
                    aRange.Borders.get_Item(Excel.XlBordersIndex.xlEdgeTop).LineStyle = Excel.XlLineStyle.xlContinuous;
                ...
                string ParameterName = ors.Rows[x][1].ToString();
                string DataType = ors.Rows[x][2].ToString();
                string DataTypeLength = ors.Rows[x][3].ToString();
                string DataTypePrecision = ors.Rows[x][4].ToString();
                string DataTypeScale = ors.Rows[x][5].ToString();
                string Nullable = ors.Rows[x][6].ToString();
                string OutputParameter = ors.Rows[x][7].ToString();
                ...
                {
                    Excel.Range AutoFitRange = excelWorkSheet.get_Range("A1:G" + currentRow.ToString());
