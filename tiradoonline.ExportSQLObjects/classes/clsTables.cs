    ...
    public class clsTables
    {
        ...
        public string GenerateTable(string table_name, SqlConnection objSQLConnection, DataTable dataTable)
        {
            try
            {
                ...
                objStringBuilder.AppendLine("   DROP TABLE " + clsSQLServer.SQLBlankSpaceEncode(table_name) + ";");
                objStringBuilder.AppendLine("GO");
                objStringBuilder.AppendLine("");
                objStringBuilder.AppendLine("CREATE TABLE " + clsSQLServer.SQLBlankSpaceEncode(table_name));
                objStringBuilder.AppendLine("(");
                ...
                for (int y = 0; y < objColumnNames.Rows.Count; y++)
                {
                    ...
                    objStringBuilder.Append("\t" + DataType + " " + clsSQLServer.SQLBlankSpaceEncode(ColumnName) + "\t\t" + GetIdentity(objSQLConnection, table_name) + " " + GetDefault(objSQLConnection, table_name, ColumnName) + " " + Nullable);

                    if (y < objColumnNames.Rows.Count - 1)
                        objStringBuilder.AppendLine(",");
                }
                objStringBuilder.AppendLine(Environment.NewLine);
                objStringBuilder.AppendLine(");");
        ...
        {
            string SQL = "select c.text AS DefaultText from sysobjects a, syscolumns b, syscomments c where a.id = b.id AND b.cdefault = c.id and a.name = '" + clsSQLServer.SQLBlankSpaceEncode(table_name) + "' and b.name = '" + clsSQLServer.SQLBlankSpaceEncode(column_name) + "'";
        ...
        private string GetIdentity(SqlConnection objSQLConnection, string tablename)
        {
            ...
            {
                identityExists = true;
                string SQL = "SELECT SEED = (SELECT IDENT_SEED('" + clsSQLServer.SQLBlankSpaceEncode(tablename) + "')), INCREMENT = (SELECT IDENT_INCR('" + clsSQLServer.SQLBlankSpaceEncode(tablename) + "'))";
                SqlCommand objSQLCommand = new SqlCommand(SQL, objSQLConnection);
                SqlDataAdapter objSqlDataAdapter = new SqlDataAdapter(objSQLCommand);
                DataTable objTableNames = new DataTable();
                objSqlDataAdapter.Fill(objTableNames);
