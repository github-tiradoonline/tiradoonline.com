...
//using ExportSQLObjects.classes;

namespace ExportSQLObjects
{
    public partial class FormExportSQLObjects : Form
    {
        ...
        private string registryFolder = @"HKEY_CURRENT_USER\Software\ExportSQLObjects";
        ...
        private void FormExportSQLObjects_Load(object sender, EventArgs e)
        {
            ...
            //textBox_Username.Text = "balanceintranet_user";
            //textBox_Password.Text = "balanceintranet";
            //textBox_Database.Text = "BalanceIntranet";
        }
                ...
                if (objTableNames != null)
                {
                    if (objTableNames.Rows.Count > 0)
                    {
                        ...
                        for (int x = 0; x < objTableNames.Rows.Count; x++)
                        {
                            ...
                            objStringBuilder.AppendLine("IF EXISTS (SELECT NAME FROM sysobjects WHERE NAME = '" + clsSQLServer.SQLBlankSpaceEncode(ProcedureName) + "' AND TYPE = 'P')");
                            objStringBuilder.AppendLine("\tDROP PROCEDURE " + clsSQLServer.SQLBlankSpaceEncode(ProcedureName) + ";");
                ...
                if (objTableNames != null)
                {
                    if (objTableNames.Rows.Count > 0)
                    {
                        ...
                        for (int x = 0; x < objTableNames.Rows.Count; x++)
                        {
                            ...
                            TableText = cls_Schema.GenerateTable(objTableNames.Rows[x]["ObjectName"].ToString(), objSQLConnection, objTableNames);
                            ...
                            if (objPK.Rows.Count > 0)
                            {
                                ...
                                for (int y = 0; y < objPK.Rows.Count; y++)
                                {
                                    ...
                                    objStringBuilder.AppendLine("");
                                    string pkstring = "ALTER TABLE " + clsSQLServer.SQLBlankSpaceEncode(TableName) + " ADD CONSTRAINT " + pk_name + " PRIMARY KEY (" + clsSQLServer.SQLBlankSpaceEncode(column_name) + ");";
                                    objStringBuilder.AppendLine("GO");
                                    objStringBuilder.AppendLine("");
                            ...
                            if (objFK.Rows.Count > 0)
                            {
                                ...
                                for (int y = 0; y < objFK.Rows.Count; y++)
                                {
                                    ...
                                    foreignkeysText += "ALTER TABLE " + clsSQLServer.SQLBlankSpaceEncode(TableName) + " ADD CONSTRAINT " + constraint_name + " FOREIGN KEY (" + clsSQLServer.SQLBlankSpaceEncode(fk_column_name) + ") REFERENCES " + clsSQLServer.SQLBlankSpaceEncode(fk_table) + "(" + clsSQLServer.SQLBlankSpaceEncode(pk_column_name) + ");" + Environment.NewLine;
                            ...
                            StreamWriter SW;
                            string objectName = objTableNames.Rows[x]["ObjectName"].ToString();
                            //if (!char.IsLetter(objectName) || !char.IsLetter(objectName))

                            DirectoryPath = DirectoryPath + "\\" + objectName + ".sql";
                            SW = File.CreateText(DirectoryPath);
                            SW.WriteLine(objStringBuilder.ToString());
                            SW.Close();
                ...
                if (objTableNames != null)
                {
                    if (objTableNames.Rows.Count > 0)
                    {
                        ...
                        for (int x = 0; x < objTableNames.Rows.Count; x++)
                        {
                            ...
                            objStringBuilder.AppendLine("\tDROP View " + clsSQLServer.SQLBlankSpaceEncode(ViewName) + ";");
                            objStringBuilder.AppendLine("GO");
                            objStringBuilder.AppendLine("");
